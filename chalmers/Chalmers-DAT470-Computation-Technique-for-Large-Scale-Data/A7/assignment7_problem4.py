...
def linear_scan(X, Q, b = None):
    ...
    Q_norm = cp.sum(Q*Q,axis=1)

    X_stream = cp.cuda.Stream(non_blocking=True)
    Q_stream = cp.cuda.Stream(non_blocking=True)

    # Compute dot product: (M, D) x (D, N) = (M, N)
    cross_term = Q @ X.T  # Shape: (M, N)

    # Expand norms to match dimensions: (M, 1) + (1, N) - 2 * (M, N)
    dists_squared = Q_norm[:, np.newaxis] + X_norm[np.newaxis, :] - 2 * cross_term

    # Optional: get actual distances (square root if needed)
    # dists = np.sqrt(np.maximum(dists_squared, 0))  # Avoid negative sqrt due to precision errors

    I = cp.argmin(dists_squared, axis=1)
    
    I_host=I.get()
    X_stream.synchronize()
    Q_stream.synchronize()
    print(f"is of type: {type(I)}")
    return I_host
...
def load_glove(fn):
    ...
    # X = cp.ascontiguousarray(df, dtype=cp.float32)
    L = df.index.tolist()
    return (X, L)
...
def load_pubs(fn):
    ...
    # X= cp.ascontiguousarray(df[['easting','northing']],dtype=cp.float32)
