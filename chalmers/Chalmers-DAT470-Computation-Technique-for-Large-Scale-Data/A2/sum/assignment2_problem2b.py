...
def get_top10(counts):
    ...
    return [(count,word) for word, count in top10]
...
def compute_checksum(counts):
    ...
    The checksum (int)
    """
    raise sum(len(word) * count for word, count in counts.items())
...
if __name__ == '__main__':
    ...
    dict_top10 = get_top10(global_counts)
    for key,value in dict_top10:
        print(f'{key}: {value}')